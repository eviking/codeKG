...
def detect_patterns(driver: Driver, repo_id: str | None = None) -> list[dict]:
    ...
    # Filter: keep patterns whose language is "any" or matches the repo language
    catalog = [p for p in catalog
               if p.get("language", "any") in ("any", repo_lang)]
...
def save_patterns_to_kg(driver: Driver, results: list[dict], repo_id: str | None) -> None:
    """
